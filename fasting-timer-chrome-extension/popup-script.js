// Fasting Timer Stats Loader - Carga directamente del backend api/stats (sin extension.min.js)

let currentDuration = 75; // Valor por defecto si localStorage está vacío

// --- INICIALIZACIÓN: Leemos lo guardado o usamos default ===
try {
  const savedDuration = localStorage.getItem('fastingTimerPreferredDuration');
  if (savedDuration) {
...
function savePreference(minutes) {
  try {
    localStorage.setItem('fastingTimerPreferredDuration', minutes.toString());
...
window.fastingTimerBackendLoaded = true;

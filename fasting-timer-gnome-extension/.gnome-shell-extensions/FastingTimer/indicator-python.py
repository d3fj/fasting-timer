...
class SmokeTimerIndicator:
    def __init__(self, uid, label):
        self._uid = uid
        self._label = label
        self.running = False
        
        # Icono simple de fuego usando emoji o SVG base64
        self.icon = 'fire.png'  # Debería estar en la carpeta icons/
        
    def connect(self, manager):
        print("SmokeTimerIndicator connecting...")
        return True
    ...
    def start(self, duration_minutes):
        ...
        if not self.running:
            ...
            window = Notify.Window.new("Smoke Timer")
            window.set_transient_for(Gtk.Application.get_default())
            
            # Icono personalizado
            icon_pixbuf = Gdk.Icon.DESKTOPACCESSIBILITY
    ...
    if window:
        ...
        if child_label and hasattr(window, 'set_text'):
            ...
            if progress_percentage % 300 == 0:
                Notify.Notification.new("Smoke Time Update", "Progress update...").show()
